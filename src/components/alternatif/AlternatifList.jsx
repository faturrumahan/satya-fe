...
const AlternatifList = ({ currentItems }) => {
  const deleteRequestHandler = async (id) => {
    try {
      const res = await fetch(`http://127.0.0.1:4000/alternatif/${id}`, {
        method: "DELETE",
      });
      if (res.status === 200) {
        ...
        {currentItems &&
          currentItems.map((item, index) => (
            ...
            >
              ...
              <td className="py-3">{item.id_jurusan}</td>
              <td className="flex space-x-5 w-full justify-center py-3">
                <Link href={`alternatif/ubah/${item.id}`}>Ubah</Link>
                <button
                  onClick={() => {
